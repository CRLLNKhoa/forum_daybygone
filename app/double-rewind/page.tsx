...
import doubleCost from "@/json/doubleCost.js";
...
export default function Double() {
  ...
  return (
    <div className="flex flex-col mt-8 bg-white w-full items-center justify-center p-6 rounded-lg">
      <h1 className="text-xl font-bold mb-4">Double Rewind</h1>
      ...
      {resultTable.length !== 0 ? (
          ...
          <TableBody>
            {resultTable.map((row) => (
              <TableRow
                key={row.day}
                className={cn(
                  "border h-[24px] border-slate-400 hover:bg-black/80 duration-500 text-white cursor-pointer",
                  row.cost >= 262 && row.cost <= 298 && "bg-[#1ac000]",
                  row.cost >= 298 && row.cost <= 334 && "bg-[#33b300]",
                  row.cost >= 334 && row.cost <= 370 && "bg-[#4d9900]",
                  row.cost >= 370 && row.cost <= 406 && "bg-[#669900]",
                  row.cost >= 406 && row.cost <= 443 && "bg-[#808000]",
                  row.cost >= 443 && row.cost <= 479 && "bg-[#996600]",
                  row.cost >= 479 && row.cost <= 515 && "bg-[#b24c00]",
                  row.cost >= 515 && row.cost <= 551 && "bg-[#cc3200]",
                  row.cost >= 551 && row.cost <= 587 && "bg-[#e51900]",
                  row.cost >= 587 && row.cost <= 624 && "bg-[#ff0000]"
                )}
              >
        ...
        <div className="flex items-center justify-center mt-8 h-[120px] flex-col">
          ...
          <h1 className="text-sm text-center">
            Enter value & click check button
          </h1>
        </div>
      )}
    </div>
